...
public class SolarSystemDatabase
{
    ...
    public Asteroid randomAstroid(String name,double minMass,double maxMass,double minDistance,double maxDistance,
                                 double minSpeed,double maxSpeed,double minRotation,double maxRotation)
    {
        ...
        Asteroid a = new Asteroid(name,c,posX,posY,mass,1,speedX,speedY,rotation);
    ...
    public SolarSystem getOurSolarSystem()
    {
        ...
        ourSolarSystem.defaultSystemSelectedMision = sun;

        //asteroid randomized
        int astroidNumber = 300;
        for(int i = 0;i<astroidNumber;i++)
        {
            Asteroid a = randomAstroid("ASTEROID#"+i,Math.pow(10, 15),2.12 * Math.pow(10, 18),2.5,2.8,17000,22000,0,0);
